...
cp sysroot/usr/share/gammaos/gammaos iso_root/boot/
...
xorriso -as mkisofs -b boot/limine-bios-cd.bin -no-emul-boot -boot-load-size 4 \
    ...
    --efi-boot-image --protective-msdos-label iso_root -o gammaos.iso

# Install limine.
host-pkgs/limine/usr/local/bin/limine bios-install gammaos.iso